...
subroutine plot_2functions (x, y1, y2, legend, title, N)

  implicit none

  real(kind=8), dimension(:), intent(in) :: x, y1, y2

  real(kind=8), dimension(size(x), 3) :: xx

  character(len = *), dimension(:), intent(in) :: legend

  integer, dimension(:,:), optional, intent(inout) :: N
  ...
  call write_xyy_data ( data_file_name, size(x), size(x), 3, xx, ierror)
  !  write_xyy_data ( data_file_name, lda, nrow, ncol, x, ierror)
...
if(present(N)) then
  ...
  call write_xyy_plots ( command_file_name, data_file_name, 3, legend, title, ierror, N )
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror, N )

else

  call write_xyy_plots ( command_file_name, data_file_name, 3, legend, title, ierror)
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror)
...
end subroutine plot_2functions

subroutine plot_3functions (x, y1, y2, y3, legend, title, N)

  implicit none

  real(kind=8), dimension(:), intent(in) :: x, y1, y2, y3

  real(kind=8), dimension(size(x), 4) :: xx
  ...
  call write_xyy_data ( data_file_name, size(x), size(x), 4, xx, ierror)
    !  write_xyy_data ( data_file_name, lda, nrow, ncol, x, ierror)
...
if(present(N)) then
  ...
  call write_xyy_plots ( command_file_name, data_file_name, 4, legend, title, ierror, N )
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror, N )

else

  call write_xyy_plots ( command_file_name, data_file_name, 4, legend, title, ierror)
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror)
...
end subroutine plot_3functions

subroutine plot_4functions (x, y1, y2, y3, y4, legend, title, N)

  implicit none

  real(kind=8), dimension(:), intent(in) :: x, y1, y2, y3, y4

  real(kind=8), dimension(size(x), 5) :: xx
  ...
  call write_xyy_data ( data_file_name, size(x), size(x), 5, xx, ierror)
  !  write_xyy_data ( data_file_name, lda, nrow, ncol, x, ierror)
...
if(present(N)) then
  ...
  call write_xyy_plots ( command_file_name, data_file_name, 5, legend, title, ierror, N )
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror, N )

else

  call write_xyy_plots ( command_file_name, data_file_name, 5, legend, title, ierror)
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror)
...
end subroutine plot_4functions

subroutine plot_muchmorethan2functions (x, yy, N, legend, title, M)

  ! x = vettore delle x ( Capitan Ovvio)
  ! yy = vettore lungo N * size(x) contenente incolonnate una sotto l'altra
  ! le y delle funzioni da plottare
  ! N = numero di funzioni da plottare
  ...
  integer, dimension(:,:), optional, intent(inout) :: M

  character(len = *), intent(in) :: title

  integer, intent(in) :: N

  real(kind=8), dimension(size(x), N + 1) :: xx
  ...
  do j = 2, (N + 1)
    ...
    xx(:, j) = yy( 1 + (j - 2) * size(x) : (j - 1) * size(x) )
 ...
 call write_xyy_data ( data_file_name, size(x), size(x), N + 1, xx, ierror)
 !  write_xyy_data ( data_file_name, lda, nrow, ncol, x, ierror)
...
if(present(M)) then
  ...
  call write_xyy_plots ( command_file_name, data_file_name, N + 1, legend, title, ierror, M )
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror, M )

else

  call write_xyy_plots ( command_file_name, data_file_name, N + 1, legend, title, ierror)
    !  write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror)
...
end subroutine plot_muchmorethan2functions


subroutine plot_function (x, y, legend, title)
  ...
  call write_xy_plot ( command_file_name, data_file_name, legend, title, ierror )
...
subroutine plot_video (XX, YY, title, legend, fn, t, save_flag )
...
!  end do
  ...
  call write_xy_plot_video (XX, YY, command_file_name, data_file_name,  &
                            legend, title, ierror, n_active_columns, fn_,&
                            t_, save_flag)
...
subroutine write_xy_data_video ( nr, nc,  x, y, ierror , data_file_name_base)
  ...
  real(kind=8), dimension(size(x, 1), size(y, 1)/size(x, 1) + 1) :: xx
...
!AR
      ...
      do s = 1, size(y, 1) / size(x, 1)

        xx(:, s + 1) = y( 1 + (s - 1) * size(x, 1) : s * size(x, 1), j)
...
subroutine write_xy_plot ( command_file_name, data_file_name, legend, title, ierror )
!
!*******************************************************************************
!
...
!
  character ( len = * ) command_file_name
  character ( len = * ) data_file_name
  character ( len = * ), dimension(:) :: legend
  character ( len = * ) title
  integer file_unit
! LQ  integer i
  integer ierror
...
!
  ...
  write ( file_unit, '(a)' ) 'set ylabel "y"'
  write ( file_unit, '(a, i2, a)' ) 'plot "' // trim ( data_file_name ) // &
                                    '" using 1:2 title "' // trim(legend(1)) //  '" with lines lc 1'
...
subroutine write_xy_plot_video (x, y, command_file_name, data_file_name, legend, title, ierror, nc, fn, t, save_flag)
  ...
  if (PRESENT(save_flag)) THEN
    if (save_flag) THEN
      write ( file_unit, '(a)' ) 'set terminal png size 1920,1080' ! 'set term png'
    end if
  end if
  ...
  do j=1, nc
      ...
      if (size(x, 1) /= size(y, 1)) then
        ...
        do o = 1, size(y, 1)/size(x, 1)
          ...
          !int_sup = int_inf + 1
          !write ( file_unit, '(a, i4, a, i4, a, f6.5, a, i1, a)' ) 'plot "' // trim ( data_file_name ) // &
          !     '" every ::',int_inf, '::',int_sup,' using 1:2 title "' // trim( legend(o) ) // &
          !     '" offset 0,',-(o-1)/5.d0,'with lines lc ',(o+1),' lw 2'

          if( o == 1) then

             write ( file_unit, '(a, i4, a, i4, a, i1, a)' ) 'plot "' // trim ( data_file_name ) // &
                  '" using 1:',(o+1),' title "' // trim(legend(o)) // &
                  '" with lines lc ',(o+1),' lw 2,\'

          elseif( o /= size(y, 1)/size(x, 1) .and. o /= 1) then

            write ( file_unit, '(a, i4, a, i4, a, i1, a)' ) '"' // trim ( data_file_name ) // &
                 '" using 1:',(o+1),' title "' // trim(legend(o)) // &
                 '" with lines lc ',(o+1),' lw 2,\'
...
subroutine write_xyy_plots ( command_file_name, data_file_name, ncol, legend, title, ierror, N )
!
!*******************************************************************************
!
...
!
  ...
  write ( file_unit, '(a)' ) 'set xlabel "x"'
  write ( file_unit, '(a)' ) 'set ylabel "y"'
  !write ( file_unit, '(a)' ) 'set label "stato sinistro" at "50, 0" offset 0, 0.5 '
  !write ( file_unit, '(a)' ) 'set label "stato destro" at "30, 0" offset 0, 0.5 '


  ! modifications by Jacopo Banchetti 26 settembre 2018
  !====================================================
  do i = 2, ncol
    ...
    if (i == 2) then
      ...
      write ( file_unit, '(a,i2,a)' ) 'plot "' // trim ( data_file_name ) // &
        '" using 1:', i,' title "' // trim(legend(1)) //  '" with lines ,\'
                                    ! to split line
    else
      ...
      if (i /= ncol) then
        ...
        if (.not.present(N)) then
          ...
          write(*,*) "Sono in i /= ncol e .not.present(N)"

         write ( file_unit, '(a,i2,a)' ) '"' //&
                                          trim ( data_file_name ) // &
           '" using 1:', i,'title "' // trim(legend( i - 1 )) //  '" with lines ,\'

        else

           if( i <= ncol - N(1, 1) ) then ! Sono prima delle funzioni non plottate del tutto

             write(*,*) "Sono in i /= ncol, present(N) e prima delle funzioni plottate in parte"

           write ( file_unit, '(a,i2,a)' ) '"' // trim ( data_file_name ) // &
                                           '" using 1:', i,'title "' //&
                                           trim(legend( i - 1 )) //  '" with lines ,\'

           else ! Sono nelle funzioni da plottare solo in parte

             write(*,*) "Sono in i /= ncol, present(N) e nelle delle funzioni plottate in parte"

             write(start, "(I0)") N(i - N(1,1), 1)
             write(end, "(I0)") N(i - N(1,1), 2)

             write ( file_unit, '(a,i2,a)' ) '"' // trim ( 'plot_function_data.txt' ) // &
                                             '" every :: ' // trim(start) // '::'// trim(end) //&
                                             ' using 1:', i,'title "' //&
                                             trim(legend( i - 1 )) //  '" with lines ,\'
        ...
        if(.not.present(N)) then
          ...
          write(*,*) "Sono in i == ncol, .not.present(N) "

           ! only when  i == ncol
          write ( file_unit, '(a,i2,a)' ) '"' // trim ( data_file_name ) // &
           '" using 1:', i,'title "' // trim(legend(size(legend))) //  '" with lines'

         else

           write(*,*) "Sono in i == ncol, present(N) "

           write(*,*) "i = ", i
           write(*,*) "N(1, 1) = ", N(1, 1)
           write(*,*) "N(i - N(1,1), 1) = ", N(i - N(1,1), 1)
           ...
           write ( file_unit, '(a,i2,a)' ) '"' // trim ( 'plot_function_data.txt' ) // &
                                           '" every :: ' // trim(start) // '::'// trim(end) //&
                                           ' using 1:', i,'title "' //&
                                           trim(legend(size(legend))) //  '" with lines'
    ...
    ! JB LQ write ( file_unit, '(a)' ) 'pause -1'
  ...
  ! end of modifications by Jacopo Banchetti 26 settembre 2018
  !===========================================================
